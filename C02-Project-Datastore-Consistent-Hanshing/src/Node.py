...
class Node:
    ...
    def get_data(self, key):
        ...
        owner_node_name = self._vnode_map.get_assigned_node(key)
        if owner_node_name is self.name :
            return self._data_store[key]
        else :
            remote_node = self._node_dict[owner_node_name]
            return remote_node.get_data(key)
    ...
    def set_data(self, key, value, force=False):
        if (force):
            ...
            if owner_node_name is self.name :
                ...
                remote_node = self._node_dict[owner_node_name]
                remote_node.set_data(key, value)
    ...
    def add_new_node(self, new_node_name, new_node):
        ...
        user_id_list = list(self._data_store.keys())
        for user_id in user_id_list :
            #Add the data if, its data is owned by vnode in the local slice
            vnode = self._vnode_map.get_assigned_vnode(user_id)
            if (vnode in local_vnode_slice) :
                #Prepare the dict item according to the format above
                transfer_dict.setdefault(vnode, {'target_node': new_node_name, 'keys': []})['keys'].append(user_id)
    ...
    def remove_current_node(self, new_node_dict):
        ...
        for user_id in user_id_list :
            vnode = self._vnode_map.get_assigned_vnode(user_id)
            #Prepare the dict item according to the format above
            transfer_dict.setdefault(vnode, {'target_node': transfer_node_mapping[vnode], 'keys': []})['keys'].append(user_id)
